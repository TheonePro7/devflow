...
try {
    ...
    # First run
    $output1 = & $psExe -NoProfile -File scripts/merge-settings.ps1 "$testDir\.claude\settings.json" 2>&1
    ...
    $output2 = & $psExe -NoProfile -File scripts/merge-settings.ps1 "$testDir\.claude\settings.json" 2>&1
...
'@ | Out-File "$testDir\hooks\guardrails-git.sh" -Encoding utf8
    ...
    $output3 = & $psExe -NoProfile -File scripts/merge-guardrails.ps1 "$testDir\hooks\guardrails-git.sh" 2>&1
    ...
    # Second run
    $output4 = & $psExe -NoProfile -File scripts/merge-guardrails.ps1 "$testDir\hooks\guardrails-git.sh" 2>&1
    $isIdle2 = $false
...
"@ | Out-File "$testDir\.gitignore" -Encoding utf8
    ...
    & $psExe -NoProfile -File scripts/merge-gitignore.ps1 "$testDir\.gitignore" 2>&1 | Out-Null
    ...
    # Second run
    $output5 = & $psExe -NoProfile -File scripts/merge-gitignore.ps1 "$testDir\.gitignore" 2>&1
    $isIdle3 = $false
...
"@ | Out-File "$testDir\docs\CONTEXT.md" -Encoding utf8
    ...
    & $psExe -NoProfile -File scripts/merge-docs.ps1 "$testDir" 2>&1 | Out-Null
    Pass "merge-docs: first run completes"

    $output6 = & $psExe -NoProfile -File scripts/merge-docs.ps1 "$testDir" 2>&1
    $isIdle4 = $false
